...
| 📚 **下载历史** | 记录已下载漫画，随时查看 |
| ⚡ **多线程加速** | 内置 30 线程并发下载图片，速度拉满 |
| ⏯️ **断点续传** | 中断后重新下载自动跳过已存在图片，只补缺失部分 |
| 🔗 **链接识别** | 直接粘贴 `https://18comic.vip/album/xxx/` 或 `JM123` 自动识别 ID |
| 🗂️ **历史管理** | 网页端查看大小、一键打开目录、删除已下载漫画 |
| 📊 **实时进度条** | 命令行 & 网页版均实时显示下载进度、已获取图片数 |
| 🌐 **网页图形界面** | 浏览器操作，带实时进度条，零学习成本 |
| 📁 **本地管理** | 所有文件保存在 exe 同目录 `downloads/`，结构清晰 |
| 🌍 **代理支持** | 支持 HTTP/HTTPS 代理，网络环境受限也能用 |
| ⏰ **定时下载** | 设定每天时间点自动下载指定漫画，自动追番不熬夜 |
| 🔔 **追更订阅** | 书签一键开启追更，自动检测新章节更新 |
| 🖼️ **封面墙** | 一键导出全部已下载漫画封面，生成书架网页 |
| 📊 **下载队列面板** | 网页端实时查看队列进度、每本成功/失败状态 |
| 🔍 **搜索历史** | 自动记录最近 10 条搜索，点击即回搜 |
| 🎨 **主题自定义** | 自定义主题色，前端即时生效 |
| 🌐 **多语言** | 中文 / English 一键切换 |
| 📱 **手机远程控制** | 局域网内手机浏览器直接操作（0.0.0.0 监听） |
...
- **v1.0.4**：🚀 9 项大升级 —— 断点续传增强（自动清理损坏半截文件+补全）· ⏰定时下载 · 🔔追更订阅（自动检测更新）· 🖼️封面墙 · 📊下载队列面板 · 🔍搜索历史 · 🌐中英双语 · 🎨主题色自定义 · 📱手机局域网远程控制
- **v1.0.3**：网页版看图画廊 · 自动检查更新横幅 · 下载失败自动重试 · 下载完成桌面通知 · 命令行 history · ⭐书签收藏 · 🌗明暗主题切换 · ⚙️代理设置界面
- **v1.0.2**：链接自动识别 ID、历史记录增强（大小/打开目录/删除）、修复残留进程占用端口问题
- **v1.0.1**：断点续传、命令行进度条、网页版进度动画、CI 自动打包
- **v1.0.0**：首个版本
